...
static uint32 copseReclaimDelay[MAX_DEATH_COUNT] = { 15, 30, 60 }; //30, 60, 120 };
        ...
        if (Pet* pet = GetPet())
        {
            if (pet && pet->IsInWorld())
            {
                for (auto const& cooldown : pet->m_CreatureSpellCooldowns)
                    SendClearCooldown(cooldown.first, pet);
...
void Player::CheckDuelDistance(time_t currTime)
{
    ...
    {
        if (!IsWithinDistInMap(obj, 80/*50*/))
        {
    ...
    {
        if (IsWithinDistInMap(obj, 70/*40*/))
        {
...
bool Player::InArenaQueue() const
{
    for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
    {
        if (_BgBattlegroundQueueID[i].bgQueueTypeId == BATTLEGROUND_QUEUE_2v2 || _BgBattlegroundQueueID[i].bgQueueTypeId == BATTLEGROUND_QUEUE_3v3 ||
            _BgBattlegroundQueueID[i].bgQueueTypeId == BATTLEGROUND_QUEUE_5v5 || _BgBattlegroundQueueID[i].bgQueueTypeId == 12/*Solo3v3*/)
        {
            return true;
        }
    }
